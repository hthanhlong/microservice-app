...
export class AuthService {
  ...
  async signUp(signUpDto: SignUpDto): Promise<IRes<SignUpResponseDto>> {
    const endpoint = this.configService.get<string>('services.auth.v1.signUp');
    if (!endpoint) {
  ...
  async signIn(signInDto: SignInDto): Promise<IRes<SignInResponseDto>> {
    const endpoint = this.configService.get<string>('services.auth.v1.signIn');
    if (!endpoint) {
  ...
  async refreshTokens(refreshToken: string): Promise<IRes<SignInResponseDto>> {
    const endpoint = this.configService.get<string>(
      'services.auth.v1.refreshTokens',
    );
    if (!endpoint) {
